...
def run_nginx(indices, doc_type='webserver', service='nginx'):
    webd = WebDetector(client, doc_type=doc_type, service=service, indices=indices)
    webd.query_es()
    webd.process_response()

def run_dovecot(indices, doc_type='mail', service='dovecot'):
    dove = DovecotDetector(client, indices=indices, doc_type=doc_type, service=service)
    dove.query_es()
    dove.process_response()

def run_sshd(indices, doc_type='auth', service='sshd'):
    sshd = SSHDetector(client, doc_type=doc_type, indices=indices, service=service)
    sshd.query_es()
    sshd.process_response()
...
if args.agg:
    ...
    if any('events' in index for index in args.indices):
        ...
        exit()
    elif args.service == 'nginx':
        run_nginx(args.indices)
    elif args.service == 'dovecot': 
        run_dovecot(args.indices)
    elif args.service == 'sshd': 
        run_sshd(args.indices)
